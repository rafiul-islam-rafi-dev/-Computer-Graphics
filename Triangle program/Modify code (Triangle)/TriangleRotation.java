...
public class TriangleRotation extends JPanel {
    ...
    // New-triangle er X-coordinate rotation korar value store korar jonno array.
    int[] xr = new int[3];
    // New-triangle er Y-coordinate rotation korar value store korar jonno array.
    int[] yr = new int[3];

    // Rotation angle. Degree theke Radian e convert kore nibo karon Math.cos() and Math.sin() radian noy.
    double angle = 45; // degree te rotation angle
    double radian = Math.toRadians(angle); // degree ke radian e convert kora hocche

    // Constructor
    // Object create hole automatic call hoy
    TriangleRotation() {
        // New Rotation apply korar jonno loop
        for (int i = 0; i < 3; i++) {
            // x-coordinate rotate kora hocche cos and sin use kore
            xr[i] = (int)(x[i] * Math.cos(radian) - y[i] * Math.sin(radian)); // Formula:  x' = x*cos(θ) - y*sin(θ)
            // y-coordinate rotate kora hocche sin and cos use kore
            yr[i] = (int)(x[i] * Math.sin(radian) + y[i] * Math.cos(radian)); // Formula:  y' = x*sin(θ) + y*cos(θ)
        }
    }
    ...
    public void paintComponent(Graphics g) {
        ...
        // New Rotated triangle (Red)
        g.setColor(Color.RED);
        // New Rotated triangle draw kora hocche.
        drawTriangle(g, xr, yr);
    }

    public static void main(String[] args) {
        // ekti Window create korlam. title "Triangle Rotation" dilam.
